...
If Not m_blnInitiated Then
    ...
    If lngResult = 0 Then
    Else
        Err.Raise lngResult, "modSocket.InitiateProcesses", GetErrorDescription(lngResult)
...
lngResult = api_WSAStartup(&H202, udtWSAData)
...
If m_blnInitiated And m_lngSocksQuantity = 0 Then
    If FinalizeService = SOCKET_ERROR Then
        ...
        Err.Raise lngErrorCode, "modSocket.FinalizeProcesses", GetErrorDescription(lngErrorCode)
    Else
    End If
...
If lngResult = 0 Then
    ...
    Err.Raise sckOutOfMemory, "modSocket.DestroyWinsockMessageWindow", "Out of memory"
Else
    m_lngWindowHandle = 0
End If
...
If m_colSocketsInst Is Nothing Then
    ...
    If CreateWinsockMessageWindow <> 0 Then
        Err.Raise sckOutOfMemory, "modSocket.RegisterSocket", "Out of memory"
    End If
...
If blnEvents Then
    ...
    If lngResult = SOCKET_ERROR Then
        lngErrorCode = Err.LastDllError
        Err.Raise lngErrorCode, "modSocket.RegisterSocket", GetErrorDescription(lngErrorCode)
    Else
    End If
End If
...
Const MOD_WS   As String = "ws2_32"                         'Location of the closesocket function
  ...
  If Subclass_InIDE Then
    ...
    api_CopyMemory ByVal nAddrSubclass + 13, &H9090, 2
    ...
    Subclass_PatchRel PATCH_01, i                           'Patch the relative address to the EbMode api function
  End If
